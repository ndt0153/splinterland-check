...
const USER_NAME_LIST = ["toshihikosong"];
function App() {
  ...
  function getLastestClaimQuestTime(data5) {
    if (data5.data && data5.data.length > 0) {
      const lastestClaim = data5.data.find(
        (item) => item.type == "claim_reward"
      );
  ...
  const checkInfo = async () => {
    ...
    function getAfkGame(data6) {
      if (data6.data && data6.data.battles.length > 0) {
        const afkGames = data6.data.battles.filter(
          (item) =>
            !item.details.team1 && item.details.loser == data6.data.player
        );
    ...
    const getBattlesResult = (data0, username) => {
      ...
      return {
        ...
        winRate: ((winCount * 100) / (todayBattles.length - drawCount)).toFixed(
          2
        ),
    ...
    <div className="container mx-auto">
      <table className="border-collapse border border-green-800 ...">
        ...
        <tbody>
          {data.map((item) => (
            <tr>
              <td className="border border-green-600 ...">{item}</td>
            </tr>
          ))}
        </tbody>
      </table>
